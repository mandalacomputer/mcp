...
const POWER_NEXT: Record<string, string> = {
  ...
  restart:
    ' wait_for_computer with until="guest" is what says when the desktop is back — and, on a computer with secrets bound, when they have been delivered again: a command run before that sees them unset.',
};
      ...
      guarded(async () => {
        ...
        try {
          ...
          if (body?.code === 'template_image_preparing' && error instanceof ConflictError) {
            ...
            const originalSupportsContinuation =
