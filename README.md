...
`sleep 130` failed at 125.2s with `timeout_s: 300` and at 125.3s with
`timeout_s: 3600`. Raising `timeout_s` buys nothing; `background: true` is the
only thing that works. The abandoned command keeps running, so the call after
one of these often reports the guest agent as busy — that is the first failure
